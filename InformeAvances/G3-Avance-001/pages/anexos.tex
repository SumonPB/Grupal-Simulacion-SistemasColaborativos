...
\subsection{Código Fuente de la Simulación en GAML}
En este anexo se presenta el código completo de la simulación desarrollada en la plataforma GAMA utilizando el lenguaje GAML (\textit{GAMA Modeling Language}). Este código carga los archivos vectoriales geográficos, inicializa los agentes de red con sus respectivas configuraciones de vulnerabilidad, y ejecuta el motor de propagación del ataque de ransomware WannaCry junto con la lógica de activación del protocolo de contingencia.

Adicionalmente, todo el código fuente y los recursos complementarios de esta simulación se encuentran alojados de manera pública en el siguiente repositorio de GitHub: \\
\url{https://github.com/Dennis290699/Grupal-Simulacion-SistemasColaborativos/tree/main/Simulacion/Grupal_Redes}

\newpage
\begin{lstlisting}[caption={Código fuente de la simulación basada en agentes (InfeccionRedes.gaml)}]
...
experiment Infeccion type: gui {
	output {
		display mapa_red {
			species sala; // 1. El fondo marron de la sala
			species connection; // 2. Los cables naranja
			species computer; // 3. Los nodos encima (asi nada los tapa)
		}
